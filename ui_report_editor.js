...
// --- MOTOR DE AUTOCORRECCIÓN Y LIMPIEZA ---
window.runAutocorrect = function(targetId) {
    ...
    function processTextNode(node) {
        ...
        // A. Codificación dañada
        const encodingMap = {
            'Ã³': 'ó', 'Ã¡': 'á', 'Ã©': 'é', 'Ã­': 'í', 'Ãº': 'ú', 'Ã±': 'ñ',
            'Ã“': 'Ó', 'Ã ': 'Á', 'Ã‰': 'É', 'Ã ': 'Í', 'Ãš': 'Ú', 'Ã‘': 'Ñ',
            'i{on': 'ión', '{on': 'ión', 'porliferaci{on': 'proliferación'
        };
        for (const [bad, good] of Object.entries(encodingMap)) {
            let splitText = text.split(bad);
            if (splitText.length > 1) {
                localCorrections += splitText.length - 1;
                text = splitText.join(good);
            }
        }

        // B. Dedos vecinos / números embebidos
        const fingerMap = [
            {regex: /([a-zA-ZáéíóúñÁÉÍÓÚÑ])3([a-zA-ZáéíóúñÁÉÍÓÚÑ])/g, rep: "$1e$2"}
        ];
        fingerMap.forEach(f => {
            let matches = text.match(f.regex);
            if (matches) {
                localCorrections += matches.length;
                text = text.replace(f.regex, f.rep);
            }
        });

        // C. Eliminación de duplicidad (words)
        let dupRegex = /\b([A-Za-zÁ-Úá-úñÑ]+)\s+\1\b/gi;
        while (dupRegex.test(text)) {
            let prevText = text;
            text = text.replace(dupRegex, function(match, p1) {
                return p1;
            });
            if (prevText !== text) {
                localCorrections++;
        ...
        for (const [bad, good] of Object.entries(dictionary)) {
            let escapeBad = bad.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            let regex = new RegExp(`\\b${escapeBad}\\b`, 'gi');
    ...
    function traverseNodes(node) {
        ...
        if (node.nodeType === 3) {
            ...
            for (let i = 0; i < node.childNodes.length; i++) {
                count += traverseNodes(node.childNodes[i]);
            }
        }
        return count;
    }

    let totalCorrections = traverseNodes(tempDiv);

    if (totalCorrections > 0) {
        editor.innerHTML = tempDiv.innerHTML;
        if (typeof window.showToast === 'function') {
            window.showToast(`Autocorrección aplicada: ${totalCorrections} cambios realizados`, "success");
        } else {
            alert(`Autocorrección aplicada: ${totalCorrections} cambios realizados`);
        }
    } else {
        if (typeof window.showToast === 'function') {
            window.showToast("El texto ya está corregido", "info");
        } else {
