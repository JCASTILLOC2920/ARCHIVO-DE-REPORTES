...
- **[2026-09-24] 🛡️ Erradicación de Código Simulado (`autoaprendizaje.py`) / Mito de Medusa e Integración Oficial del Modelo de Caché de Roland Kuhn & Renato De Mori (1990)**: Se erradicó por completo el código simulado previo de `autoaprendizaje.py` (+5000 hiper-bono) y el mito de Medusa. Se implementó e integró oficialmente la arquitectura matemática del Modelo de Lenguaje de Caché de Roland Kuhn & Renato De Mori (1990) con decaimiento exponencial adaptado al Dictáfono clínico (\(\alpha = 0.04, \lambda = 0.65\), normalización Z-score). Sistema verificado, sincronizado y en estado `OPERATIVO_CERTIFICADO`.

- **[2026-09-23] 🛡️ Solución Definitiva de Visibilidad para las 6 Plantillas de Quistes de Lester en Reportes**: Se implementó la auto-sanitización V14 (`templates_quistes_v14_injected`) en `db_service.js` para forzar la verificación e inyección de las 12 plantillas de quistes (IDs 1040-1045 y 1140-1145) en `templatesDatabase` y guardarlas en `localStorage.plantillasDB`. Asimismo, en `ui_report_editor.js` (`actualizarPlantillasSegunEspecialidad`), se garantizó que las 6 plantillas de Quistes de Lester aparezcan SIEMPRE visibles en los desplegables `#re_planMacro` y `#re_planMicro`, organizadas bajo un `<optgroup label="--- QUISTES DE OVARIO (LESTER) ---">` superior y separadas de `<optgroup label="--- OTRAS PLANTILLAS ---">`, con priorización automática si se selecciona Ginecología o si el órgano/muestra incluye la palabra 'QUISTE'.
